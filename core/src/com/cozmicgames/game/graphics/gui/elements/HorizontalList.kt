...
class HorizontalList<T : GUIElement>(style: ListStyle = ListStyle()) : List<T>(style) {
    ...
    override fun add(element: T) {
        ...
        val node = Node(size)
        size++
        node.constraints.x = previousNode?.let { absolute { it.x + it.width - x } } ?: same()
        node.constraints.y = same()
    ...
    override fun remove(index: Int) {
        ...
        nextNode.constraints.x = previousNode?.let { absolute { it.x + it.width - x } } ?: same()
    }
}